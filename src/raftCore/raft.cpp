...
// 通过Start向leader添加命令，leader会生成新的日志
...
/*需要持久化的数据有：
1. raft节点的状态：当前节点的任期，当前任期给谁投过票，节点保存的所有日志信息
2. 快照：快照最新包含哪个日志index，快照最新包含哪个term

怎么进行持久化？
用boost库中的序列化工具boost::archive::text_oarchive，定义一个结构体将需要持久化的数据集中在一个对象中，将关键状态序列化为字符串格式
需要的时候可以通过反序列化解析出来
*/
